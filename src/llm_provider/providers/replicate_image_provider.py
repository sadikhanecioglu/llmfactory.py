...
class ReplicateImageProvider(BaseImageProvider):
    ...
    def __init__(self, config: ReplicateImageConfig):
        ...
        if not REPLICATE_AVAILABLE:
            raise ImportError(
                "Replicate package is required for Replicate image provider"
            )
    ...
    async def generate_image(self, prompt: str, **kwargs) -> ImageResponse:
        ...
        try:
            ...
            logger.info(
                f"🎨 Generating image with Replicate: prompt='{prompt[:50]}...', model={model}"
            )
            ...
            # Case 2: List of string URLs
                ...
                for item in output:
                    ...
                    # List of dict objects with url field
                        ...
                        urls.append(item.url)
                        logger.info(f"URL from object attribute: {item.url}")
